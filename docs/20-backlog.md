...
| T70 | Compare two cards from the board | The comparison has a screen, reached from the card that would be cloned. | merged |
...
| T76 | Carry the new signals into the digest | Cost and contradictions, both read from the database. Anything needing a subprocess per card was left out. | merged |
