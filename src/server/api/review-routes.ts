    ...
    async (request, reply) => {
      try {
        ...
        if (source === null || source === undefined) {
          return conflict(
            reply,
            'This card has no branch to publish. Dispatch it before opening a pull request.',
          );
        }
        ...
        if (base === null)
          return conflict(
            reply,
            'The board could not determine a base branch. Set one on the card.',
          );
        if (base === source)
          return conflict(
            reply,
            'The source and base branch are the same. Change one before opening a pull request.',
          );

        const report =
          card.completionReport === null ? '' : `\n\n## Agent report\n\n${card.completionReport}`;
